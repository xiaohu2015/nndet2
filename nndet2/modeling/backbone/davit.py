"""
modified from https://github.com/dingmyu/davit/blob/main/mmdet/mmdet/models/backbones/davit.py
"""
...
class DaViT(Backbone):
    ...
    def __init__(self, in_chans=3, depths=(1, 1, 3, 1), patch_size=4,
                 ...
                 attention_types=('spatial', 'channel'), ffn=True, overlapped_patch=False, cpe_act=False,
                 out_features=None
                 ):
        super().__init__()
        ...
        self._out_feature_channels = {stage: self.embed_dims[int(stage[-1]) - 1] for stage in self._out_features}

        # add a norm layer for each output
        min_stage_index = int(self._out_features[0][-1]) - 1
        for i_layer in range(min_stage_index, self.num_stages):
